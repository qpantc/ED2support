...
#----- Default settings. ------------------------------------------------------------------#
setwd('~/projects/ED2.2/MPED2_Support/tower_processing/Fluxsites/FLX_GF-Guy_FLUX_2004-2020/')
...
for (p in sequence(nplaces)){
   ...
   file.in    = file.path(here,paste0(info$name))
   #---------------------------------------------------------------------------------------#
   ...
   #     Find the dates.                                                                   #
   #---------------------------------------------------------------------------------------#
   datum$time = ymd_hm(datum$TIMESTAMP_START)
   datum$year <- year(datum$time)
   
   datum$month <- month(datum$time)
   datum$day <- day(datum$time)
   datum$hour <- hour(datum$time)
   datum$min <- minute(datum$time)
   datum$sec <- second(datum$time)
